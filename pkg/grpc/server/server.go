...
func (s ArticleServer) Create(ctx context.Context, req *pb.CreateArticleRequest) (_ *pb.CreateArticleResponse, err error) {
	...
	if err != nil {
		return nil, status.Errorf(codes.Internal, "Failed to make article-created event: %v", err)
	}

	if err := s.broker.ResilientPublish(event); err != nil {
		return nil, status.Errorf(codes.Internal, "Failed to publish the article-created event: %v", err)
	}
...
func (s ArticleServer) Delete(ctx context.Context, req *pb.DeleteArticleRequest) (_ *emptypb.Empty, err error) {
	...
	if err != nil {
		return nil, status.Errorf(codes.Internal, "Failed to make the article-deleted event: %v", err)
	}

	if err := s.broker.ResilientPublish(event); err != nil {
		return nil, status.Errorf(codes.Internal, "Failed to publish the article-deleted event: %v", err)
	}
...
func (s ArticleServer) Update(ctx context.Context, req *pb.UpdateArticleRequest) (_ *emptypb.Empty, err error) {
	...
	if err != nil {
		return nil, status.Errorf(codes.Internal, "Failed to make the article-updated event: %v", err)
	}

	if err := s.broker.ResilientPublish(event); err != nil {
		return nil, status.Errorf(codes.Internal, "Failed to publish the article-updated event: %v", err)
	}
